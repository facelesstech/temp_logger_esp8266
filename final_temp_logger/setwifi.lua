...
wifi.setmode(wifi.STATIONAP) -- Wifi config mode
cfg={}
  cfg.ssid="Temp logger" -- SSID
  --cfg.password="12345678" --comment to leave open
wifi.ap.config(cfg)

ipcfg={}
  ipcfg.ip="192.168.1.1" -- Set up page
...
srv:listen(80,function(conn)
  conn:on("receive", function(client,request)
    ...
    if (path == "/" and  vars == nil) then
      ...
      buf = buf.."<h1>Temp logger Wifi Configuration</h1>"
    ...
    elseif (vars ~= nil) then
      restarting = "<html><body style='width:90%;margin-left:auto;margin-right:auto;background-color:White;'><h1>Restarting...You may close this window.</h1></body></html>"
